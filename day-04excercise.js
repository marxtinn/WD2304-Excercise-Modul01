...
const triangle = (height) => {
  // triangle merupakan fungsi tetap setelah melalui const. height merupakan parameter dari fungsi triangle.
  ...
  for (let i = 0; i < height; i++) {
    // loop height hingga sekian kali (sesuai value height)
    for (let j = 0; j <= i; j++) {
      // loop hingga j = loop j = 0 -> current height
      temp++; // menambah temp sebelum result
      if (temp > 9) {
        // apabila result lebih besar dari 9, maka tidak akan ditambahkan "0" sebelum angka
...
function fizzBuzz(n) {
  // definisikan fungsi fizzBuzz dengan variable n (n pada saat console log dapat diubah sesuka hati)
  let j = ""; // j adalah empty string untuk menampung value yang dihasilkan setelah for loop (temp variable)
  for (let i = 0; i < n; ) {
    // melakukan looping dari 0 -> n(input)
...
function calculateBMI(w, h) {
  // menentukan nama fungsi dan parameter fungsi
  let BMI = w / h ** 2; // definisikan rumus BMI yaitu w dibagi dengan h pangkat 2
  console.log(BMI);
  if (BMI > 39.9) {
    // menentukan kondisi dengan parameter tertentu dalam hal ini jika nilai dari BMI lebih besar dari 39.9, maka
    message = "obesity"; // akan memberi output "obesity"
    console.log(message);
  } else if (BMI >= 30.0) {
    // apabila kriteria pada kondisi if tidak terpenuhi maka akan dilanjutkan dengan statement else if berikutnya
...
let remove = numbers.filter(function (num) {
  // menentukan fungsi dengan built in method filter untuk menentukan bilangan ganjil dan bilangan genap
...
const splitString = (str) => {
  ...
  for (var i = 0; i < str.length; i++) {
    // melakukan looping untuk setiap index dari string yang diisi, mulai dari index ke-0. Loop nya bakal berjalan terus sampai akhir string.
    if (str[i] !== " ") {
      // jika karakter pada posisi index (i) adalah bukan spasi,
      temp += str[i]; // maka karakter dalam index i akan dimasukkan ke temp. temp itu adalah string kosong.
    } else {
      // kalau karakter dalam posisi i adalah spasi, kita menggunakan built in method push untuk
    ...
    if (i == str.length - 1)
      // jika i adalah huruf terakhir, maka akan di push dari temp ke outputArr.
      outputArr.push(temp);
  }
